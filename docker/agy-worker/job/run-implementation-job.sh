...
write_job() {
  ...
  jq -n --arg job_id "${JOB_ID}" --arg status "${status}" --arg phase "${phase}" \
    --arg updated_at "${now}" --arg error "${error}" \
    '{job_id:$job_id,task_type:"implement",status:$status,phase:$phase,updated_at:$updated_at,error:(if $error == "" then null else $error end)}' \
    > "${JOB_FILE}.tmp"
...
while IFS= read -r change; do
  ...
  if grep -Eqi '(gh[pousr]_[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_-]{20,}|-----BEGIN [A-Z ]*PRIVATE KEY-----|Bearer[[:space:]]+[A-Za-z0-9._~+/=-]{20,})' "${target}.tmp"; then
    rm -f "${target}.tmp"
    fail_job "Potential secret detected in proposed changes"
  fi
...
while IFS= read -r line; do
  ...
  if [[ "${status}" == *D* ]]; then
    ...
    size="$(wc -c < "${WORKTREE}/${path}")"
    sha256="$(sha256sum "${WORKTREE}/${path}" | cut -d' ' -f1)"
  fi
  ${first} || printf ',' >> "${MANIFEST_FILE}.tmp"
  first=false
